...
export default function Dashboard() {
  ...
  const fetchConversations = async () => {
    ...
    try {
      ...
      const response = await fetch("https://aggieworks-backend.bardia.app" + '/conversations', {
        headers: {
  ...
  const refreshConversations = async () => {
    ...
    try {
      const token = localStorage.getItem('token')
      const response = await fetch("https://aggieworks-backend.bardia.app" + '/query-bucket', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      })
  ...
  const handleSearch = async () => {
    ...
    try {
      const token = localStorage.getItem('token')
      const response = await fetch("https://aggieworks-backend.bardia.app" + `/search?q=${encodeURIComponent(globalSearch)}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      })
