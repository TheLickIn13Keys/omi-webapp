...
export default function ConversationView({ conversation }: { conversation: Conversation }) {
  ...
  const fetchConversationDetails = async () => {
    try {
      ...
      const response = await fetch("https://aggieworks-backend.bardia.app" + `/conversations/${conversation.id}`, {
        headers: {
  ...
  const fetchAudioFile = async () => {
    try {
      const token = localStorage.getItem('token')
      const response = await fetch("https://aggieworks-backend.bardia.app" + `/conversations/${conversation.id}/audio`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      })
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const token = localStorage.getItem('token')
      const response = await fetch("https://aggieworks-backend.bardia.app" + `/conversations/${conversation.id}/messages`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
